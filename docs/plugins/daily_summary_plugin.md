...
- **跨周期连贯性**：通过 `history_count` 参数，每次生成简报时可携带最近 N 次的历史简报上下文，让大模型感知此前趋势（例如连续三天的游戏动态），生成更连贯的总结
...
history_count = 3                   # 可选：生成简报时携带最近 N 次的历史简报作为大模型上下文，0 表示不携带
```
